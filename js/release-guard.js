...
const REQUIRED_IDS=['app','loadingScreen','menuScreen','periodScreen','profileScreen','skirmishScreen','campaignScreen','gameScreen','modalOverlay','recoveryOverlay','gameCanvas','minimapCanvas','btnNewGame','btnSkirmishMenu','btnContinue','btnSettings','btnCredits','btnLanguageMenu','languageOverlay','btnPeriodConfirm','btnDiplomacyPanel','btnCityAdminPanel','btnProvincePanel','btnRoyalCouncilPanel','btnCourtIntriguePanel','btnSecretOpsPanel','btnInfoWarfarePanel','btnCulturalMoralePanel','btnReligionCivilizationPanel','btnInternalEconomyPanel','btnImperialLogisticsPanel','btnPublicHealthPanel','btnEducationKnowledgePanel','btnExternalCommercePanel','diplomacyOverlay','btnDiplomacyClose','btnRecoverRestart','btnRecoverSafe','btnRecoverBack'];
...
function migrateSave(save){
  ...
  if(!migrated.religionCivilization||typeof migrated.religionCivilization!=='object')migrated.religionCivilization=null;
  if(!migrated.internalEconomy||typeof migrated.internalEconomy!=='object')migrated.internalEconomy=null;
  if(!migrated.imperialLogistics||typeof migrated.imperialLogistics!=='object')migrated.imperialLogistics=null;
  if(!migrated.publicHealth||typeof migrated.publicHealth!=='object')migrated.publicHealth=null;
  if(!migrated.educationKnowledge||typeof migrated.educationKnowledge!=='object')migrated.educationKnowledge=null;
  if(!migrated.justiceLaw||typeof migrated.justiceLaw!=='object')migrated.justiceLaw=null;
  if(!migrated.demographyUrbanism||typeof migrated.demographyUrbanism!=='object')migrated.demographyUrbanism=null;
  if(!migrated.externalCommerce||typeof migrated.externalCommerce!=='object')migrated.externalCommerce=null;
  migrated.schemaVersion=27;
  if(original<24){migrated.migratedFrom=original;migrated.migratedAt=new Date().toISOString();addEvent('save-migration',`Save migrado do schema ${original} para 27`,{eraPack:migrated.eraPack,gameMode:migrated.gameMode});}
  return migrated;
}
function captureStorage(){for(const key of ['valeEmpires.save','valeEmpires.save.campaign','valeEmpires.save.skirmish','valeEmpires.save.backup','valeEmpires.save.campaign.backup','valeEmpires.save.skirmish.backup','valeEmpires.profile','valeEmpires.campaign','valeEmpires.eraPack','valeEmpires.audioPrefs','valeEmpires.lastSaveAt','valeEmpires.language','valeEmpires.skirmish','valeEmpires.secretOperations','valeEmpires.informationWarfare','valeEmpires.culturalMorale','valeEmpires.religionCivilization','valeEmpires.internalEconomy','valeEmpires.imperialLogistics','valeEmpires.publicHealth','valeEmpires.educationKnowledge','valeEmpires.justiceLaw','valeEmpires.demographyUrbanism','valeEmpires.externalCommerce']){let raw=null;try{raw=localStorage.getItem(key)}catch(err){REPORT.warnings.push(`Storage indisponível: ${err.message}`)}REPORT.storage[key]={present:raw!==null,bytes:raw?new Blob([raw]).size:0,validJSON:true};if(raw&&!['valeEmpires.lastSaveAt','valeEmpires.language','valeEmpires.eraPack'].includes(key)){try{JSON.parse(raw)}catch{REPORT.storage[key].validJSON=false;REPORT.warnings.push(`JSON inválido em ${key}`)}}}}
