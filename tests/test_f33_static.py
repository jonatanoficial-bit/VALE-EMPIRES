...
if build.get('version')!='v3.9.0': errors.append('versão incorreta')
if build.get('saveSchema')!=13: errors.append('save schema incorreto')
app=(root/'js/app.js').read_text(encoding='utf-8')
for token in ['defaultCityAdministration','serializeCityAdministration','initializeCityAdministration','updateCityAdministration','spawnCityRevolt','openCityAdministrationPanel','schemaVersion:13','cityAdministration:serializeCityAdministration()']:
...
required=['btnCityAdminPanel','cityStabilityStat','cityLoyaltyStat','cityGarrisonStat','cityRebuildStat','cityRevoltStat','cityIntegrationStat']
for rid in required:
...
(root/'tests/RELATORIO_TESTE_ESTATICO_F33.json').write_text(json.dumps(report,indent=2,ensure_ascii=False),encoding='utf-8')
